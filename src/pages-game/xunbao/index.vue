	...
	<view class="oh page box">
		...
		<view class="add_container flex-ali-col" v-show="quan_tc">
			...
			<view class="add_container2">
				<u-line-progress :percentage="percentage" :showText="false" activeColor="#FA7700" height="18"></u-line-progress>
			</view>
		...
		<view class="container" v-show="!quan_tc">
			...
			<view class="container_bj">
				<u-image :src="ASSETSURL + 'image/ad_load11.png'" width="750rpx" height="1418rpx" v-if="youx_zdao_id == 5"></u-image>
				<!-- <image :src="ASSETSURL + 'ad_1.png'"
					  style="width: 100vw;height: 100vh; position: absolute; left: 0; bottom: 0;bottom: -40rpx;" v-else>
				  </image> -->
				<view :style="{ opacity: effEShow ? '1' : '0' }" style="width: 100vw; height: 100vh; position: absolute; left: 0; bottom: 0; bottom: 0rpx; transition: all 0.5s" v-else>
			...
			<view class="youx_zdao" v-if="youx_zdao" @click="youx_zdao_xyb">
				...
				<block v-for="(item, index) in 8" :key="index">
					<image :src="`${ASSETSURL}guide/${index + 1}.png`" :class="{ cur: youx_zdao_id == index + 1 }" style="width: 100vw; height: 100vh"></image>
				</block>
			</view>
			<!-- 弹窗语音识别 -->
			<dy-record v-if="showRecord" :showRecord="showRecord" @close="showRecord = false" :task="taskList[1]" ref="record" @detailCloce="detailCloce"></dy-record>
			<!-- 活动规则 -->
			<u-popup :show="show_2" mode="center" :safeAreaInsetBottom="false" bgColor="transparent" @click="show_2 = false" :overlayOpacity="0.8">
				<view class="warper">
					<view class="warper1">
			...
			<!-- 分享弹窗 -->
			<u-popup :show="show_3" mode="center" :safeAreaInsetBottom="false" bgColor="transparent" @click="show_3 = false" :overlayOpacity="0.8">
				<view class="warper">
					<view class="warperTanC">
						...
						<view class="yaoQin" @click="shareF">
			...
			<dy-prize ref="dyPrize2" seus="1" :show="showPrize" :item="prizeDetail" @close="showPrize = false" @getZlyq="showPrize = false"></dy-prize>
			<!-- 寻找宝藏 -->
			<u-popup :show="show_1" :safeAreaInsetBottom="false" bgColor="transparent" @click="show_1 = false" :overlayOpacity="overlayOpacity">
					...
					<view class="arCard">
						...
						<view class="arContent">
							<view class="flex-bet" style="margin-bottom: 20rpx">
								...
								<dy-button fontSize="24" width="122rpx" btn_bg="#847b73" height="50rpx" v-if="scan.finishTimes == scan.totalTimes">已完成</dy-button>
								<dy-button fontSize="24" width="122rpx" height="50rpx" @click="arCard" v-else>去完成</dy-button>
							</view>
							<u-line-progress :percentage="(scan.finishTimes / scan.totalTimes) * 100" :showText="false" height="14rpx" activeColor="#FA7700" inactiveColor="#000000"></u-line-progress>
						</view>
					</view>
							...
							<view class="task" :style="{ 'background-image': `url(${ASSETSURL + item.icon}.png)` }">
								...
								<view style="margin-top: 6rpx">
									<block v-if="item.todaySignStatus">
										<u-line-progress :percentage="100" :showText="false" height="14rpx" activeColor="#FA7700" inactiveColor="#000000" v-if="item.todaySignStatus"></u-line-progress>
										<u-line-progress :percentage="0" :showText="false" height="14rpx" activeColor="#FA7700" inactiveColor="#000000" v-else></u-line-progress>
									</block>
									<block v-else>
										<u-line-progress :percentage="(item.now / item.max) * 100" :showText="false" height="14rpx" activeColor="#FA7700" inactiveColor="#000000"></u-line-progress>
									</block>
								</view>
								...
								<view class="flex-bet">
									...
									<dy-button fontSize="20" width="92rpx" height="36rpx" btn_bg="#847b73" v-if="item.todaySignStatus">{{ item.btnList[1] }}</dy-button>
									<dy-button fontSize="20" width="92rpx" height="36rpx" @click="$u.route(item.page), getSignin(item, index)" v-else-if="item.now < item.max">
										{{ item.btnList[0] }}
									</dy-button>
									<dy-button fontSize="20" width="92rpx" height="36rpx" btn_bg="#847b73" v-else>{{ item.btnList[1] }}</dy-button>
								</view>
							</view>
						</block>
					</view>
...
export default {
	...
	onLoad(ope) {
		...
		// }
		this.add_jsq()
		reportExposeEvent({ activityName: '游戏首页浏览', actionRank: 0, activityId: 'game_xunbao_home_view', activityContent: { qudaoCode: ope.qudaoCode } })
	},
	methods: {
		shareF(){
			reportClickEvent({ activityName: '邀请好友', actionRank: 0, activityId: 'game_xunbao_home_click_share', activityContent: {} })
		},
		getScene(scene) {
		...
		handleDoubleClick1() {
			...
			if (diff < 300) {
				...
				reportClickEvent({ activityName: '首页埋点点击', actionRank: 0, activityId: 'game_xunbao_click_point', activityContent: {} })
		...
		//签到 分享等
		getSignin(e, inx) {
			reportClickEvent({ activityName: '去完成任务', actionRank: 0, activityId: 'game_xunbao_index_click_task', activityContent: { name: e.name } })
			console.log(e, '------------')
			// someClickEvent() 全局埋点
			if (e.id == 4) {
