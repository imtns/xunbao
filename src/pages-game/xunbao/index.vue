            ...
            <block v-for="(item, index) in taskList" :key="index">
              <view class="task" :style="{ 'background-image': `url(${ASSETSURL + item.icon}.png)` }">
                ...
                <view class="flex-bet">
                  ...
                  <dy-button fontSize="20" width="92rpx" height="36rpx" @click="$u.route(item.page), getSignin(item, index)" v-else-if="item.now < item.max || (item.id == 4 && item.now == item.max)">
                    {{ item.btnList[0] }}
                  </dy-button>
...
export default {
  ...
  methods: {
    ...
    handleDoubleClick1() {
      if (!this.isLogin) return tool.jump_nav('/pages-sub/login/index')
      // 单击或双击
      let _this = this
