<template>
  <view class="oh page box">
    ...
    <view class="container" v-show="!quan_tc">
      ...
      <!-- 弹窗语音识别 -->
      <dy-record v-if="showRecord" :showRecord="showRecord" @close="showRecord = false" ref="record" @detailCloce="detailCloce"></dy-record>
      <!-- 活动规则 -->
          ...
          <view class="flex-bet" style="margin-top: 38rpx">
            <block v-for="(item, index) in taskList" :key="index">
              <view class="task" :style="{ 'background-image': `url(${ASSETSURL + item.icon}.png)` }">
                <view class="taskName">
                  {{ item.name }}
                  <text v-if="item.id != 4">({{ item.now }}/{{ item.max }})</text>
                </view>
                ...
                <view class="taskText">答对问题，集卡 片赢奖励</view>
                <view class="flex-bet">
  ...
  methods: {
    ...
    getSignin(e, inx) {
      console.log(e, '------------')
        ...
        //   reportClickEvent({
        //     activityName: '签到点击/分享点击/xx去完成点击', // 写中文能看的懂什么地方点击的
        //     actionRank: 1,
        //     activityId: 'some_card_click', // 埋点英文id，自定义根据页面点击方法自己定义，能看的懂就行
        //     activityContent: data, // 请求的参数
        //   })
      if (e.id == 4) {
