		...
		function startDraw() {
			...
			for (var n in options.imgList) {
				...
				if (!options.imgList[n].radius) {
					...
					ctx.save()
					handleBorderRect(ctx, options.imgList[n].imgX, options.imgList[n].imgY, options.imgList[
							n].imgW, options
				...
				function drawImg(arc) {
					...
					if (_rotate) {
						...
						ctx.translate((options.imgList[n].imgX + rx) * -1, (options.imgList[n].imgY + ry) *
							-1)
					}
				}
			}
			...
			function drawFont() {
				...
				for (let k in fonts) {
					...
					if (fonts[k].vel) {
						...
						ctx.fillText(fonts[k].string, _rotate ? 0 : fonts[k].textX, _rotate ? 0 : fonts[k]
							.textY)
					}
					if (_rotate) {
