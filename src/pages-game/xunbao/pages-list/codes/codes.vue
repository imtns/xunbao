<template>
  <view class="codes">
    ...
    <dy-prize :show="showPrize" :item="prizeDetail" v-if="code != 10007" @close=";(showPrize = false), arInit()" @getZlyq=";(showPrize = false), arInit()"></dy-prize>
  </view>
</template>
    ...
    arInit() {
      ...
      this.listener = context.onCameraFrame((frame) => {
        ...
        if (index % 20 == 0 && this.isSend) {
          ...
          let that = this
          tool.uploadFiles([this.base64ToImg(base64)], 'https://java.vrupup.com/identify/link/uploadFile').then((res) => {
