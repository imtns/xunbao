			...
			<block v-for="(item, index) in detail.optionJson" :key="index">
				...
				<view class="add_cur0" :class="{ add_cur1: dq_cw == 2 || dq_cw == 1 }">
					<view class="option" @click="index_cur(index)" :class="{ cur: cur == index }">
						...
						<image class="optionImg" :src="`${ASSETSURL}dayAnswer/${index + 1}.png`" mode="widthFix">
						</image>
						<image class="star add1" :src="`${ASSETSURL}dayAnswer/dd.png`" mode="widthFix"
							v-if="dq_zq == index"></image>
