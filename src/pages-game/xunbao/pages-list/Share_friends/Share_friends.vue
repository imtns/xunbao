	...
	<view class="Share">
		...
		<view style="position: relative; z-index: 33">
			...
			<view v-if="Share_type == 2" style="margin-top: 65rpx">
				...
				<view class="Share1_1"
					>送你一张<text>{{ type }}</text
					>，赶快点击<text>收下</text>集齐 4张卡片就可以获得宝藏唱片音响1份还有 更多隐藏奖励等你挖掘！
				</view>
				<view class="Share2 tc">
					<image mode="widthFix" :src="`${ASSETSURL}Share2.png`"></image>
				</view>
...
export default {
	...
	onLoad(ope) {
		if (!this.isLogin) {
			reportClickEvent({ activityName: '好友分享页跳转登录', actionRank: 0, activityId: 'game_xunbao_share_click_login', activityContent: {} })
