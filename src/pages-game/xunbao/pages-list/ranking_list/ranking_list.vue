	...
	<view class="ranking_list">
		<view class="rankingceng">
			...
			<view class="ranking" :style="{'padding-top': showRanking ? '162rpx' : 0}">
				<view class="ranking1 oh flex-bet" :class="dataList.length == 0 ? '' : 'shows'"
					v-if="dataList.length && isGetDateOk">
				...
				<view class="ranking2 oh" :class="listShow || dataList.length == 0 ? 'listShow' : ''">
					...
					<view v-if="dataList.length == 0 && isGetDateOk" class="ranking21_1">
						<image mode="widthFix" :src="`${ASSETSURL}wu_zt2.png`" style="width: 325rpx;"></image>
					</view>
				</view>

				<view class="ranking2 ranking2_lo" v-if="isGetDateOk">
...
<style>
	@media screen and (max-height: 720px) {
		.ranking_list .rankingceng .ranking {
			transform: scale(0.85) translateY(-100rpx);
		}
	}
</style>