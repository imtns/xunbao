<template>
  <view class="lime-painter" ref="limepainter">
    <view v-if="canvasId && size" :style="styles">
      ...
      <!-- #endif -->
      <!-- #ifdef APP-NVUE -->
      <web-view :style="size" ref="webview" src="/uni_modules/lime-painter/static/index.html" class="lime-painter__canvas" @pagefinish="onPageFinish" @error="onError" @onPostMessage="onMessage">
      </web-view>
      <!-- #endif -->
    </view>
    <slot />
  ...
  methods: {
    ...
    canvasToTempFilePath(args = {}) {
      return new Promise(async (resolve, reject) => {
        ...
        let destWidth = Math.round(width * dpr)
        let destHeight = Math.round(height * dpr)
        // #ifdef MP-ALIPAY
        width = destWidth
        height = destHeight
        // #endif
